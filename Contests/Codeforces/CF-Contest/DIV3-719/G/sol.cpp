...
ll ask(ll l , ll r){
    auto it = queries.find(r - 1);
    if(it != queries.end()){
        ll val = (it -> s);
        
        val += query(1 , 0 , n - 1 , 0 , r - 1);
        return val;
    }
    ...
    queries[r - 1] = ans - query(1 , 0 , n - 1 , 0 , r - 1);
    return ans;
}
...
void solve(){
    ...
    while(t--){
        ...
        while(l <= r){
            ll mid = l + (r - l)/2;
            ll ones = ask(1 , mid + 1);
            // save values of the first array

            ll zeroes = mid + 1 - ones;
          
            if(zeroes >= k) ans = mid, r = mid - 1;
            else{
                l = mid + 1;
            }
        }
