...
int main(){
    ...
    for(ll i=0;i<k;i++){
        if(i<(k+1)/2) min_heap.insert(first_window[i]);
        else max_heap.insert(first_window[i]);
    }
    ...
    for(ll i=k;i<n;i++){
        ...
        if(a[i]<=median) min_heap.insert(a[i]);
        else max_heap.insert(a[i]);

        // a[i-k] <= median -> delete from min heap
        if(a[i-k]<=median) min_heap.erase(min_heap.find(a[i-k]));
        else max_heap.erase(max_heap.find(a[i-k]));
