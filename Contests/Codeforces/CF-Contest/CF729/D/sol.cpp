...
public:
    string ANS(string &X, string &Y) {
        ...
        int maxlen = 0;
        int endingIndex = m;
        vector<vector<int>> lookup(m + 2, vector<int>(n + 2, 0));
        for (int i = 1; i <= m; i++) {
            for (int j = 1; j <= n; j++) {
                if (X[i - 1] == Y[j - 1]) {
        ...
        if (endingIndex - maxlen < 0 || maxlen >= n) {
            cout << X << " " << Y << "\n";
            cout << endingIndex - maxlen << " ";
            cout << maxlen << " ";
            return "";
        }
...
void print(int i, int j)
    ...
    string getLongestCommonSubstring()
    {
        ...
        for (k = 0; k < maxHeight; k++)
            ans.push_back(text[k + substringStartIndex]);
    ...
    int longestCommonSubpath(int N, vector<vector<int>>& paths) {
        ...
        string ans = str[0];
        for (int i = 1; i < n; i++) {
            strcpy(text, ans + "#" + str[i] + "$"); buildSuffixTree();
            ans = getLongestCommonSubstring();
            // cout << i << " " << ans << "\n";
        }
